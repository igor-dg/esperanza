...
onMounted(async () => {
  // Primero cargar la configuración de anuncios
  try {
  ...
  // Comprueba si el anuncio está activo en la configuración
  const isModalActive = announcementStore.meetingModalActive;
  
  // Comprueba si el usuario ha elegido no mostrar el modal de nuevo
  const isHidden = localStorage.getItem(STORAGE_KEY);
  
  // Solo mostrar si está activo y el usuario no lo ha ocultado
  if (isModalActive && !isHidden) {
    setTimeout(() => {
      isOpen.value = true;
